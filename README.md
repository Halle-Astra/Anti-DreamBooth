...
## revision for P40 on WSL(Ubuntu22.04)

!!!OK, 'Cause the storage of autodl.com is too small so that I cannot download the model files from the hugging face. I have to forgive the solution of autodl.com. 

Fortunatelly, I find my P40 server can run this code! Last week, It even always run into the process collapse, but it is ok now. I don't know the reason, but it is a good news. So, the following work will all the work on P40. It may help people who do like me so.

### Problems

1. OOM
	1. I revise the script from 512 resolution to 256. 
	2. Before the inference of `train_dreambooth.py`, empty cache is helpful significantly.	
	3. turn off the all mixed precision, 'Cause the P40 unsupport the FP16 and BF16. The FP16 capability of P40 is using the more memory with the FP32  to emulate the FP16 so that it will cosume with 1.5 times of the mem cost in FP32.
2. Collapse
	1. If your DIMM mem is full and the process was went into a collapse, you need clear the mem by `ps aux, kill -9 <pid>`. If the virtual mem is still too high, you can run `sudo su`, `echo 3 > /proc/sys/vm/drop_caches` and wait a few seconds for mem releasing. Then, rerun the script may be ok.
3. libcudnn\_cu8.0.so was not not found
	1. If the cuda and cudnn were installed correctly, this problem must can be solved by `sudo apt install nvidia-driver-<xxx>`.
4. Xformers should be enabled?
	1. If your device is the same as mine(P40), you must disable the xformers because of the absence of bf16 supporting.
