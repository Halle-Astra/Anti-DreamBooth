...
def main(args):
    ...
    INFO("tokenizer model loaded")

    noise_scheduler = DDPMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler")
    INFO("DDPM scheduler model loaded")
    vae = AutoencoderKL.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision
    ).cuda()
    INFO("VAE model loaded")
    ...
    INFO("clean data loaded")
    perturbed_data = load_data(
        args.instance_data_dir_for_adversarial,
        size=args.resolution,
        center_crop=args.center_crop,
    )
    INFO("perturbed data loaded")
    original_data = perturbed_data.clone()
    original_data.requires_grad_(False)
    INFO("Enable Xformers?")
    if args.enable_xformers_memory_efficient_attention:
        INFO("Try opening the xformers")
        if is_xformers_available():
            unet.enable_xformers_memory_efficient_attention()
            INFO("Opening the xformers successfully")
        else:
